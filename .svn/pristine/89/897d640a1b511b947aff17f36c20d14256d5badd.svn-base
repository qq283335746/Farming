define(['app', 'js/utils/tips'],function (app, Tips) {
    app.controller('rentOutCtrl', ['$scope', '$ionicHistory', 'Storage', '$state', 'httpRequest', function($scope, $ionicHistory, Storage, $state, httpRequest) {
        $scope.type = "home";
        ...
        $scope.fetch = function(isFresh){
            ...
            var url = "api/lease/getrentallist?";
            httpRequest.get(url, params, function (re) {
                //console.log('getrentallist--' + JSON.stringify(re));
        ...
        $scope.getDetail = function(item){
            if(!item)return;
            Storage.set("DM_TmpRentOutData", item);
            location.href = "#/rentOutPush"
        };
